const filterReducer = (state, action) => {
  switch (action.type) {
    case "LOAD_FILTER_PRODUCTS":
      let priceArr = action.payload.map((currentItem) => currentItem.price);
      console.log(
    ...
    case "FILTER_PRODUCTS":
      ...
      if (text) {
        tempFilterProduct = tempFilterProduct.filter((currentItem) => {
          return currentItem.name.toLowerCase().includes(text);
        });
      }
      ...
      if (company !== "all") {
        tempFilterProduct = tempFilterProduct.filter(
          (currentItem) => currentItem.company.toLowerCase() === company.toLowerCase()
        );
      }

      if (color !== "all") {
        tempFilterProduct = tempFilterProduct.filter((currentItem) =>
          currentItem.colors.includes(color)
        );
      }
